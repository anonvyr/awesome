...
--beautiful.init("/home/pablo/.config/awesome/themes/multicolor/theme.lua")
...
--run_once ("xrdb /home/pablo/.Xresources")
--run_once ("compton -cC -fF -I 0.065 -O 0.065 -D 10 -m 1.0")
run_once ("nm-applet")
--run_once ("mpd")
...
--    musicwidget.paht_to_icons = "~/.config/awesome/awesompd/icons"
    ...
    --musicwidget.mpd_config = "/home/c9l13s9c18e5/.config/mpd/mpd.conf"
    -- Browser (Jamendo, etc.)
...
-- Textclock
mytextclock = awful.widget.textclock("<span color='#2aa198'> %H:%M</span> ")
...
cpuwidget = lain.widgets.cpu({
    settings = function()
        widget:set_markup(markup("#859900", " CPU ") .. cpu_now.usage .. "%")
    end
})
...
memwidget = lain.widgets.mem({
    settings = function()
        widget:set_markup(markup("#6c71c4", " RAM ") .. mem_now.used .. " Mb")
    end
})
...
volumewidget = lain.widgets.alsa({
    settings = function()
        ...
        widget:set_markup(markup("#cb4b16", header) .. vlevel)
    end
})
...
for s = 1, screen.count() do
    ...
    -- Create the wibox
    mywibox[s] = awful.wibox({ position = "top", screen = s, bg = "#211D21"})
   ...
   -- right_layout:add(musicwidget.widget)
    ...
    mybbox[s] = awful.wibox({ position = "bottom", screen = s, bg = "#211D21"})


    -- Left widgets
    -- local left_bbox = wibox.layout.fixed.horizontal()
    -- left_bbox:add(mylauncher)


    -- All together
    local bbox_layout = wibox.layout.align.horizontal()
    bbox_layout:set_middle(mytasklist[s])
    bbox_layout:set_right(mylayoutbox[s])

    mybbox[s]:set_widget(bbox_layout)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "l", function () awful.util.spawn("i3lock -c 211d21", false) end),
...
awful.rules.rules = {
    ...
    -- Set terminal to always map on tag number 2 of screen 1
    { rule = { class = "URxvt" },
      properties = { function(c)
        awful.client.movetotag(tags[mouse.screen][2], c )
	awful.tag.viewonly(tags[mouse.screen][2]) end } }, 
    -- Oblogout a pantalla completa
    { rule = { class = "Oblogout" },
      properties = { fullscreen = true }},
    -- Pavucontrol siempre flotando por el espacio
